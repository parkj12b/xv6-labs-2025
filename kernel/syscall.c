...
static uint64 (*syscalls[])(void) = {
    ...
    [SYS_close] sys_close,   [SYS_interpose] sys_interpose,
};
...
void syscall(void) {
  ...
  if ((p->syscall_mask & (1 << num)) != 0) {
    if (num != SYS_open && num != SYS_exec)
      goto disallowed_syscall;

    char arg_path[MAXPATH];

    copyinstr(p->pagetable, arg_path, p->trapframe->a0, MAXPATH);
    if (strncmp(arg_path, p->interpose_path, MAXPATH) != 0)
      goto disallowed_syscall;
  }
